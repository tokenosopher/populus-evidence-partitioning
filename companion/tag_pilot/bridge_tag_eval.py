"""Run-3V TAGGED evaluator (role-marker scout, 2026-08-24).

Derived from bridge_run3v_evaluator.py with ONE change: --eval-arm
forces the evaluation mask independently of the checkpoint's training
arm (train-mask x eval-mask crossover prescribed by the deep research
audit). Frozen evaluation math untouched.
...
P_ARGS.add_argument("--eval-arm", required=True, choices=["PT", "GT"])
A = P_ARGS.parse_args()
...
_im = lm.tok(MINE_MARK, add_special_tokens=False)["input_ids"]
_io = lm.tok(OTHER_MARK, add_special_tokens=False)["input_ids"]
assert len(_im) == 1 and len(_io) == 1
MINE_ID, OTHER_ID = _im[0], _io[0]
...
TRAIN_ARM = ck["arm"]
assert TRAIN_ARM in ("PT", "GT")
ARM = A.eval_arm  # native tagged evaluation arm
assert ARM in ("PT", "GT")
assert ARM == TRAIN_ARM, "tag evaluator runs native-arm only"
...
def package(s_ids, s_mask, n):
    """TAGGED per-cell layout mirroring the tagged trainer exactly."""
    ...
    for c in range(4):
        ...
        for j in range(4):
            ...
            mkcol = torch.full((n, 1), mk, device=dev,
                               dtype=s_ids.dtype)
            slot_i.append(torch.cat([mkcol, s_ids[:, j]], dim=1))
            mono = torch.ones(n, 1, device=dev, dtype=s_mask.dtype)
            slot_m.append(torch.cat([mono, s_mask[:, j]], dim=1))
        ci = torch.cat(slot_i, dim=1)
...
rep = {"ckpt": A.ckpt, "train_arm": TRAIN_ARM, "eval_arm": ARM,
...
tag = f"F_{TRAIN_ARM}_m{rep['model_seed']}_o{rep['order_seed']}"
Path("results/run3v").mkdir(parents=True, exist_ok=True)
Path(f"results/run3v/tag_eval_{tag}.json").write_text(
