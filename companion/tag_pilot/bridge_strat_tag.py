...
TAGGED = ARM in ("PT", "GT")
if TAGGED:
    _im = lm.tok(" mine", add_special_tokens=False)["input_ids"]
    _io = lm.tok(" other", add_special_tokens=False)["input_ids"]
...
def package(s_ids, s_mask, n):
    ...
    if TAGGED:
        ...
        return (torch.cat(cells_i, dim=1),
                torch.cat(cells_m, dim=1))
    flat_i = s_ids.reshape(n, 1, 4 * Ts).expand(n, 4, -1).contiguous()
    if ARM == "G":
        flat_m = s_mask.reshape(n, 1, 4 * Ts) \
